...
# 监测日期
this_date = '20220719'  # 本工作日日期：这个一定要填写，不能为空
last_work_day = '20220718'  # 上一工作日日期：可以为空；如果为空，代码自动获取
municipal_date = '20220718'  # 本周获取城投债大全的日期：可以为空；如果为空，代码自动获取本周一的日期
...
while True:
    save_check = input("请确认数据加载完毕，保存并关闭弹出文件（完成后输入“已完成”并回车）: ")
    if save_check == "已完成":
        break
...
# 3.1
df_all_perpetual_copy = df_all_perpetual.copy()        ###
...
df_all_perpetual.to_excel(path_edited_debentures, index=False)
